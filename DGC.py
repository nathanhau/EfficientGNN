...
from models import dgc_precompute, DGC
...
def main(args):
    ...
    model = DGC(features.size(1), labels.max().item()+1, args.T, args.K)
    model = model.cuda() if args.cuda else model

    # train logistic regression and collect test accuracy
    model, train_time = train(model, adj, features[idx_train], labels[idx_train], args.epochs, args.weight_decay, args.lr)
...
def train(model,
        adj,train_features, train_labels,
