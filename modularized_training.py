...
if __name__ == "__main__":
  ...
  parser.add_argument("--model", type=str, choices=["sgc","ssgc","dgc","sgc_res"], help="sgc|ssgc|dgc|sgc_res")
  ...
  if args.mode=="linear":
    ...
    ln=nn.Linear(raw_features.shape[1],n_classes)
    train(ln,graph,precomputed,label,split_idx['train_mask'],split_idx['val_mask'],args.lr,float(args.wd),is_linear=True)
    test(ln,graph,precomputed,label,split_idx['test_mask'])
